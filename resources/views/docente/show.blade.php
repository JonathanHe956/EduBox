                    ...
                    <div>
                        ...
                        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-blue-900 dark:text-white">{{ \Carbon\Carbon::parse($docente->fecha_nacimiento)->format('d/m/Y') }}</dd>
                            </div>
                        </dl>
                    </div>
                    ...
                    <div>
                        ...
                        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <dd class="mt-1 text-sm text-blue-900 dark:text-white">{{ $docente->email ?? 'No asignado' }}</dd>
                            </div>
                        </dl>
                    </div>
                </div>
        ...
        <div class="glass-card p-6 max-w-4xl mx-auto w-full">
            <div class="border-b border-blue-200 pb-4 mb-6 dark:border-blue-700 flex justify-between items-center">
                <div>
                    ...
                    <p class="text-sm text-blue-600 dark:text-blue-300">Carga académica: {{ $docente->materias->count() }}/5 materias</p>
                </div>
            </div>
            ...
            @if($docente->materias->count() >= 5)
                <div class="mb-6 p-4 bg-yellow-50 border-l-4 border-yellow-400 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400">
                    ...
                    <p class="text-sm">El docente ya tiene asignado el máximo de 5 materias.</p>
                </div>
            @else
                <div class="mb-8 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-6 border border-blue-100 dark:border-blue-800">
                    <h3 class="text-md font-medium text-blue-900 dark:text-white mb-4">Asignar Nueva Materia</h3>
                    <form action="{{ route('docente.assignMateria', $docente) }}" method="post" class="flex flex-col sm:flex-row gap-4 items-end">
                        @csrf
                        @if(session('error'))
            ...
            <div class="overflow-hidden rounded-lg border border-blue-200 dark:border-blue-800">
                <table class="min-w-full divide-y divide-blue-200 dark:divide-blue-800">
                    ...
                    <tbody class="bg-white divide-y divide-blue-100 dark:bg-zinc-900 dark:divide-blue-800">
                        @forelse ($docente->materias as $materia)
                            <tr class="hover:bg-blue-50/50 dark:hover:bg-blue-900/20 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <form action="{{ route('docente.unassignMateria', ['docente' => $docente->id, 'materia' => $materia->id]) }}" method="post" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('¿Estás seguro de desasignar esta materia?')" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 transition-colors">
                                            Desasignar
                                        </button>
                                    </form>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-10 text-center text-blue-500 dark:text-blue-400">
                                    <p class="text-lg">No hay materias asignadas</p>
                                    <p class="text-sm mt-1">Utiliza el formulario de arriba para asignar materias.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
