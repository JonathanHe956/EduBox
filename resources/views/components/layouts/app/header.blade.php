    ...
    <body class="min-h-screen bg-blue-900 authenticated">
        <flux:header container class="border-b border-gold-500/30 bg-blue-900/95 backdrop-blur-md dark:border-gold-500/30 dark:bg-blue-900/95">
            ...
            <flux:navbar class="-mb-px max-lg:hidden">
                ...
                @if(auth()->user()->hasRole('estudiante') || auth()->user()->hasRole('docente'))
                    <flux:navbar.item icon="academic-cap" :href="route('mis.materias')" :current="request()->routeIs('mis.materias')" wire:navigate>
                        {{ __('Mis Materias') }}
                    </flux:navbar.item>
                @endif
            </flux:navbar>
        ...
        <flux:sidebar stashable sticky class="lg:hidden border-e border-gold-500/30 bg-blue-900/95 backdrop-blur-md dark:border-gold-500/30 dark:bg-blue-900/95">
            ...
            <flux:navlist variant="outline">
                <form method="POST" action="{{ route('logout') }}" class="w-full">
                    @csrf
                    <flux:navlist.item as="button" type="submit" icon="arrow-right-start-on-rectangle" class="w-full" data-test="logout-button">
                        {{ __('Log Out') }}
                    </flux:navlist.item>
                </form>
            </flux:navlist>
