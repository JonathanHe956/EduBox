                ...
                <tbody>
                    @foreach($examenes as $ex)
                        <tr class="border-t">
                            ...
                            <td class="px-4 py-2">
                                <a href="{{ route('exams.show', $ex) }}" class="text-sm text-blue-600 mr-2">Ver</a>
                                <a href="{{ route('exams.edit', $ex) }}" class="text-sm text-yellow-600 mr-2">Editar</a>
                                <form action="{{ route('exams.destroy', $ex) }}" method="POST" style="display:inline" onsubmit="return confirm('Eliminar examen?');">
                                    @csrf
                                    @method('DELETE')
